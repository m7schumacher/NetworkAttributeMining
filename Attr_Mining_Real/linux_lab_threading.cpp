...
bool isGlobalMaximal(Pattern &patt)
{
	...
	for (Pattern p : storage[patt.lastNode()])
	{
		if (p.Size() <= size){ continue; }

		if (patt.isSubset(p.getCode()))
		{
			return false;
		}
	}
...
void output()
{
	...
	//sort(true_max.begin(), true_max.end(), [](Pattern f, Pattern s) { return f.getNodes()[0] > s.getNodes()[0]; });
...
int main(int argc, char *argv[])
{
	...
	//sort(maximal.begin(), maximal.end(), [](Pattern f, Pattern s) { return s.Size() > f.Size(); });

	//for (Pattern patt : maximal)
	//{
	//	cout << patt.toString() << endl;
	//}
	...
	/*for (int i = 0; i < storage.size(); i++)
	{
		...
		{
			true_max.push_back(storage[i][j]);
		}
	}*/
	...
	//sort(true_max.begin(), true_max.end(), [](Pattern f, Pattern s) { return s.firstNode() > f.firstNode(); });

	//for (Pattern patt : true_max)
	//{
	//	cout << patt.toString() << endl;
	//}

	//cout << true_max.size() << endl;
